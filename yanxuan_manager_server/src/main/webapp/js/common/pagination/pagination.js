...
pagination.directive("pagination", function () {
    return {
        ...
        scope: false ,// 作用域，默认值为false；可选值：false(与所在模块共享作用域)，true(继承所在模块的作用域，并创建新的作用域), Object(不适用所在模块的作用域)
        link : function (scope, element, attr) {
            ...
            scope.$watch(
                ...
                function (newVal, oldVal, scope) {
                    // 当页码值发生变化时触发onChange事件，为了避免第一次页面加载发生两次请求 添加去除第一次请求的判断 :oldVal!=="0_1"
                    // 当修改页面显示记录时，也触发onChange方法 此时如果是第一页也是需要触发
                    if(newVal.toString().indexOf("...")===-1 && oldVal.toString().indexOf("...")===-1){
                        if(newVal !== oldVal && oldVal.toString().indexOf("0_1")===-1 ){
